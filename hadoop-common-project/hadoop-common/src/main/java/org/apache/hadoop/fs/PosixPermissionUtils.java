...
public class PosixPermissionUtils {
	...
	public static Set<PosixFilePermission> setPosixPermission(Path workingDir, Path p, FsPermission permission)
			throws IOException {
		...
		try {
			...
			if (uri.getScheme() == null) {
				...
				if (!path.startsWith("/")) {
					nioPath = Paths.get(workingDir.toUri().getRawPath(), path);
				} else {
					uri = new URI("file:///" + path);
					nioPath = Paths.get(uri.getRawPath());
				}
